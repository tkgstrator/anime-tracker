...
async function fetchAllPages(
  ...
  if (saveEach) {
    ...
    const cachePath = `.cache/amazon-browse/titles/${filename}.json`
    const existing = Bun.file(cachePath)
    if (await existing.exists()) {
      const cached: Title[] = await existing.json()
      ...
      console.log(`\n--- Pass: ${label} --- SKIP (cached: ${cached.length} titles, merged ${allTitles.length} unique)`)
      return
    }
  }
  ...
  if (!browseRes.ok) {
    console.error(`  Browse page error: ${browseRes.status} — skipping`)
    return
  }
  ...
  if (!paginationParams) {
    console.error('  paginationTargetId / serviceToken not found in HTML — skipping')
    return
  }
  const { paginationTargetId, serviceToken: initialPaginationToken } = paginationParams
  let currentToken = initialPaginationToken
  let startIndex = 0
  const passTitles: Title[] = []
  ...
  if (saveEach) {
    const filename = label.replace(/\s+/g, '_')
    const path = `.cache/amazon-browse/titles/${filename}.json`
    await Bun.write(path, JSON.stringify(passTitles, null, 2) + '\n')
    console.log(`  -> Saved ${passTitles.length} titles to ${path}`)
  }
}
...
const isAll = values.all || values.offer === 'all'
if (isAll) mkdirSync('.cache/amazon-browse/titles', { recursive: true })

if (values.token) {
  // --token 指定時は単一パスで実行
  await fetchAllPages({} as BuildOptions, `token=${values.token.slice(0, 20)}...`, seen, allTitles)
} else if (isAll) {
  // --all または --offer all: 全パス実行（各パスの結果も個別保存）
  for (const pass of ALL_PASSES) {
    await fetchAllPages(pass.opts, pass.label, seen, allTitles, true)
  }
} else {
  // 個別指定
...
const defaultOut = isAll ? '.cache/amazon-browse/titles.json' : '__tests__/fixtures/amazon/all.json'
const outPath = values.out ?? defaultOut
