...
| SAO映画 | **0** | 7216 | - | Amazon映画はduration=0 |
| 聲の形 | **0** | - | 7782 | Amazon映画はduration=0 |

- **Amazon は映画の duration を 0 で返す**（ウィジェット API からエピソード情報が取得できず、フォールバックエピソードが使われるため）。
...
| SAO映画 | *(fetch時刻)* | 2021-10-07T15:00:00.000Z | - |
| 聲の形 | *(fetch時刻)* | - | 2023-06-09T00:00:00.000Z |

- 全プロバイダ **ISO 8601 UTC** (`Z` サフィックス) に統一済み。
- Amazon (TV): 元データは JST 日付（`YYYY年M月D日`）で、`dayjs.tz('Asia/Tokyo')` で UTC に変換。時刻は `00:00:00 JST` = `15:00:00Z` 固定。
- Hulu: UTC を返す。Hulu 配信開始時刻（JST 23:00 or 01:00 頃）。
- Crunchyroll: UTC を返す。旧作は配信開始日が異なることがある（パンストは旧作版の日付）。
- **Amazon (映画): ウィジェット API からエピソードが取得できずフォールバックエピソードになるため、fetch 時刻が入る。**
...
| duration | ほぼ一致 | ±1秒。Amazon映画は0 |
| releaseDate | ほぼ一致 | 全プロバイダ UTC 統一済み。Amazon映画はfetch時刻 |
...
3. **duration** — ±数秒の誤差を許容すれば突き合わせに利用可能
4. **releaseDate** — UTC 統一済み。日付レベルでの比較は可能（映画を除く）
